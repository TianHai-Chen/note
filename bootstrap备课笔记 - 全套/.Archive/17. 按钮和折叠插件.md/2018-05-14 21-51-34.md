...
```html
		<!--
			1.写个按扭,写个DIV容器
			2.按扭data-toggle="collapse"
			3.容器DIV class="collapse",默认是隐藏,如一打开页面要显示,则写成class="collapse in"
			4.把容器DIV加个id="content",在按扭中写个data-target="#content"把它们关联起来.
		-->
		<button class="btn btn-default btn-group-justified" data-toggle="collapse" data-target="#content">折叠效果</button>
		<!--collapse in  :默认是隐藏的,加上in就是显示出来.不能什么都不写,不然第一次点击无效-->
...
<a href="#collapseOne" data-toggle="collapse" data-parent="#accordion">点击我进行展示，再点击我进行折叠第一部分</a>
</h4>
</div>
